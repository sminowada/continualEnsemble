...
class LifeHD():
    def __init__(self, opt, train_loader, val_loader,
                 num_classes, model, logger, device, nlearners):
        self.opt = opt
        ...
        self.model = [copy.deepcopy(model) for learner in range(nlearners)]   #change
            ...
            for idx, (image, label) in enumerate(self.train_loader):
                ...
                if idx > self.opt.warmup_batches and idx % val_freq == 0:
                    ...
                    if idx > self.opt.warmup_batches + 1 and self.opt.merge_mode != 'no_trim':
                        for model in self.models:   #change
                            model.trim_clusters()
                ...
                # Check if warmup has ended
                    if not self.warmup_done:
                        ...
                        model.classify.weight[:] = F.normalize(self.model.classify_weights)
